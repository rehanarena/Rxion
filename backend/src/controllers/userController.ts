...
const refreshAccessToken = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.error(error);
    res.status(403).json({ success: false, message: error.message || "Invalid or expired refresh token" });
  }
};
...
const changePassword = async (req: RequestWithUser, res: Response): Promise<void> => {
  try {
    ...
    const authService = new AuthService();
    const message = await authService.changePassword(userId, currentPassword, newPassword, confirmPassword);
...
//   }
// };
const getWalletBalance = async (req: CustomRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id || req.body.userId;
    if (!userId) {
